...
const setInputState = (inputElement, isValid, options) => {
	...
	if (isValid) { // если форма валидна, isValid === true
...
const toggleButtonState = (inputs, submitElement) => {
    const formIsValid = inputs.every(inputElement => inputElement.validity.valid); // вернёт false, если хотя бы один элемент будет false
...
const setEventListeners = (form, options) => {
	...
	inputs.forEach(inputElement => {
		inputElement.addEventListener('input', () => { // навешиваем слушатель на каждый инпут
